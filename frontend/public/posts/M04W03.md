...
Update the ensemble prediction: \(F_1(x) = F_0(x) + \eta \cdot f_1(x)\).

- For Age \(\le 42\):
  \[ F_1 = 236.67 + 0.5 \times (-22.11) = 225.61 \]

- For Age \(> 42\):
  \[ F_1 = 236.67 + 0.5 \times 13.27 = 243.30 \]

And that completes one iteration of XGBoost. To continue, compute new gradients and Hessians using these updated predictions and repeat the tree-building process.

---

## Final notes & intuition

- **Why second-order (Hessian) information?** Using both gradient and Hessian gives more information about the loss surface and leads to more accurate split scoring and leaf-weight estimation than using only gradients.
- **Regularization (\(\gamma,\lambda\))**: \(\gamma\) discourages creating many small leaves; \(\lambda\) shrinks leaf weights. Together they help prevent overfitting.
- **Learning rate (\(\eta\))**: scales the contribution of each tree. Smaller \(\eta\) often improves generalization (but requires more trees).

If you want, I can also:

- provide a minimal-from-scratch Python implementation (toy XGBoost for small datasets),
- add diagrams for split evaluation and leaf-weight computation, or
- export this Markdown to a `.md` file for download.
